  ...
  return (
    <div className="flex justify-center mt-8">
      <div className="bg-gray-800 p-4 rounded-lg shadow-lg ">
        <div className="flex">
          <div className="flex flex-col items-center justify-center bg-green-600 w-24 h-full rounded-l-lg">
            <div className="text-white text-2xl font-bold p-8 cursor-pointer" onClick={() => handleCellClick(0)}>0</div>
          </div>
          <div className="grid grid-cols-12 gap-4 p-4 bg-gray-700 rounded-r-lg">
            {numberRows[0].map((number) => (
              <div key={number} className={`${getColorClass(number)} text-white text-2xl font-bold flex items-center justify-center rounded cursor-pointer h-10 w-10`} onClick={() => handleCellClick(number)}>{number}</div>
            ))}
